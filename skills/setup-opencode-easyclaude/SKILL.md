...
compatibility: 需要 Node.js、npm、bun（omo 安装器用 bunx）。Windows 用 Git Bash（Unix 语法）。已在 Windows 10 + opencode 1.17.13 + oh-my-openagent 验证。
---
...
opencode --version   # 已验证 1.17.13
```
> Windows 坑：更新时可能报 `EPERM ... opencode.exe` 清理警告 —— exe 被占用导致临时目录没删，**不影响主安装**。先关掉运行中的 opencode，残留目录手动清：
...
```
安装器会：把插件写进 opencode.json 的 plugin 数组、生成 `oh-my-openagent.jsonc`（agent 映射）、生成 `tui.json`。

**坑二**：安装器生成的 `oh-my-openagent.jsonc` 里模型全是 `openai/gpt-5.4-pro`、`openai/gpt-5.3-codex` —— **既用错 provider 前缀（应是 `easyclaude/`），中转里也没这些模型**。照原样插件跑不起来，**必须用第 6 步覆盖它**。
...
## 6. ⚠️ 坑四：覆盖 `oh-my-openagent.jsonc`（模型映射最佳实践）
...
用 [assets/oh-my-openagent.jsonc](assets/oh-my-openagent.jsonc) 覆盖 `$CFG/oh-my-openagent.jsonc`。
...
```
校验 opencode.json / oh-my-openagent.jsonc 合法性与模型引用、中转连通、omo doctor、opencode 识别的 provider。
...
| omo 插件不生效 / 用了假模型 403 | 坑二：oh-my-openagent.jsonc 还是安装器默认的 `openai/gpt-5.4-pro`，用第 6 步模板覆盖 |
| 覆盖后仍是旧配置 | 坑三：删掉旧名 `oh-my-opencode.json[c]` |
