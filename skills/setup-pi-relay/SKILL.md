...
```jsonc
{
  ...
  "models": {
    "opus":    { "id": "claude-opus-4-8", "via": "anthropic", "reasoning": true, ... },  // via 决定走哪个 provider；reasoning 见下
    "gptHigh": { "id": "gpt-5.5", "via": "openai", ... },
    "gptCheap":{ "id": "gpt-5.4", "via": "openai", ... },
    "gpt56":   { "id": "gpt-5.6-sol", "via": "openai", ... }   // 可选，5.6 主打编码；七牛 id 须带 openai/ 前缀
  },
...
>
> ⚠️ **opus 的 `reasoning` 字段**（渲染进 anthropic provider 的 opus 模型）：pi 在 `reasoning:true` 时会给 `/v1/messages` 发 `thinking.enabled`。有的中转的新版 Claude（如七牛的 `claude-opus-4-8`）**拒绝该参数**，报 `"thinking.enabled" is not supported, use "adaptive" and "output_config.effort"` → 这类模型 profile 里设 `"reasoning": false`。EasyClaude 的 opus-4-8 接受旧格式，用 `true`。缺省视作 `true`。
...
| `__OPUS_ID__` / `__GPTHIGH_ID__` / `__GPTCHEAP_ID__` / `__GPT56_ID__` 等 | 对应角色 id/name（`gpt56` 角色可选；profile 无此角色则删掉模板里对应 model 对象） |
| `__OPUS_REASONING__` | `profile.models.opus.reasoning`（裸 `true`/`false`，无引号）；缺省 `true`。七牛 opus-4-8 须 `false`（见第 0 步 ⚠️） |
